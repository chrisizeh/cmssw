...
namespace ALPAKA_ACCELERATOR_NAMESPACE::torch {
   ...
   */
  ModelJitAlpaka::ModelJitAlpaka(const std::string &model_path) : cms::torch::ModelJit(model_path) {}
   ...
   */
  ModelJitAlpaka::ModelJitAlpaka(const std::string &model_path, const Device &dev)
   ...
   *        use `model.to(device, true)` to load asynchronously
   */
  ModelJitAlpaka::ModelJitAlpaka(const std::string &model_path, const Queue &queue)
