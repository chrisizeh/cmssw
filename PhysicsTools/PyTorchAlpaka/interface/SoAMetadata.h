...
namespace torch_alpaka {
    ...
    struct Block {
      ...
      Block() : columns(0), ptr(nullptr) {}
      Block(void* ptr_, const Columns& columns_, torch::ScalarType type_, size_t bytes_) : columns(columns_), ptr(ptr_), type(type_), bytes(bytes_) {}
  
      Block(void* ptr_, torch::ScalarType type_, size_t bytes_) : columns(1), ptr(ptr_), type(type_), bytes(bytes_) {
        isScalar = true;
      }

      template <typename SOA_Layout>
      std::vector<long int> get_stride(int nElements) {
    ...
    public:
      ...
      SoAMetadata(std::byte* ptr, const std::vector<torch::ScalarType>& types, const std::vector<Columns>& columns, std::vector<int>&& order_) {
        ...
        for (int i = 0; i < N; i++) {
          ...
          if(order_[i] != -1) {
            if(columns[i][0] > 0)
              blocks.try_emplace(std::to_string(i), ptr+skip, columns[i], types[i], bytes);
            else
              blocks.try_emplace(std::to_string(i), ptr+skip, types[i], bytes);

            order[order_[i]] = std::to_string(i);
            nBlocks += 1;

            auto stride = blocks[std::to_string(i)].template get_stride<SOA_Layout>(1);
            skip += columns[i][0] * stride[1] * bytes;
          } else {
            Block block;
            if(columns[i][0] > 0)
              block = Block(ptr, columns[i], types[i], bytes);
            else
              block = Block(ptr, types[i], bytes);

            auto stride = block.template get_stride<SOA_Layout>(1);
            skip += columns[i][0] * stride[1] * bytes;
          }
      ...
      SoAMetadata(void* ptr, const torch::ScalarType types, const Columns& columns) {
        nBlocks = 1;
    
        size_t bytes = torch::elementSize(types);
        blocks.try_emplace("0", ptr, columns, types, bytes);
        order.push_back("0");
      }

      template <typename T, int rows, int cols>
      void append_eigen_block(std::string name, const int columns, Eigen::Map<Eigen::Matrix<T, rows, cols>, 0, Eigen::InnerStride<>> ptr) {
        void* p = &ptr(0, 0);
      ...
      void change_order(const std::vector<std::string>& new_order) {
        order = new_order;
      }
