...
namespace cms::torch::alpakatools {
  ...
  public:
    ...
    void add(const std::string& name,
      ...
      int n_elems = cms::torch::alpakatools::detail::num_elements_per_column(batch_size, SoALayout::alignment, sizeof(DataType));
      assert_location(
    ...
    // SOA_COLUMN
    template <typename SoALayout, typename TSoAParamsImpl, typename... Others>
      requires(SameScalarType<TSoAParamsImpl, Others...> && TSoAParamsImpl::columnType == cms::soa::SoAColumnType::column)
    void add(const std::string& name,
                         int batch_size,
                         std::tuple<TSoAParamsImpl, cms::soa::size_type> column,
                         std::tuple<Others, cms::soa::size_type>... others) {
    ...
    cms::torch::alpakatools::detail::ITensorHandle<TQueue>& operator[](const size_t index) const { return *registry_.at(order_[index]); }
  ...
  private:
    ...
    void emplace_tensor(const std::string& name,
      ...
      registry_.try_emplace(
          name,
          std::make_unique<cms::torch::alpakatools::detail::TensorHandle<TQueue, T>>(alignment, sizeof(T), ptr, batch_size, std::move(dims), is_scalar));
