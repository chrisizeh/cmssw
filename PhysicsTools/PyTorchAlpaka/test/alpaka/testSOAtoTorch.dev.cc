...
namespace ALPAKA_ACCELERATOR_NAMESPACE::torchtest {
  ...
  std::string testSOAToTorch::script() const { return "create_linear_dnn.py"; }
  ...
  void testSOAToTorch::test() {
    ...
    auto model = ModelJitAlpaka(model_path);
    model.to(queue);

    // Create SoA Metadata
    ::cms::torch::SoAMetadata<SoAPosition> input(batch_size);
    auto posview = positionCollection.view().records();
    input.append_block("main", posview.x(), posview.y(), posview.z());

    ::cms::torch::SoAMetadata<SoAResult> output(batch_size);
    auto view = resultCollection.view().records();
    output.append_block("result", view.x(), view.y());
    ::cms::torch::ModelMetadata metadata(input, output);
