...
The `TensorRegistry` can be defined by first initializing the object and then adding data blocks with `register_tensor` to the internal metadata. Each registered tensor is transformed into a PyTorch tensor (without taking ownership) whose size and type are derived from the columns provided.
...
input.register_tensor<SoA>("eigen_matrix", records.c());
