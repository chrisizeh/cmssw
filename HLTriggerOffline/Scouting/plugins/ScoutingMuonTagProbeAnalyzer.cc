...
ScoutingMuonTagProbeAnalyzer::ScoutingMuonTagProbeAnalyzer(
    ...
    : outputInternalPath_(
      ...
      scoutingMuonCollection_(consumes<std::vector<Run3ScoutingMuon>>(
          iConfig.getParameter<edm::InputTag>("ScoutingMuonCollection"))),
      scoutingVtxCollection_(consumes<std::vector<Run3ScoutingVertex>>(
          iConfig.getParameter<edm::InputTag>("ScoutingVtxCollection"))),
...
void ScoutingMuonTagProbeAnalyzer::dqmAnalyze(
  ...
  for (const auto& sct_mu : *sctMuons) {
    ...
    math::PtEtaPhiMLorentzVector tag_sct_mu(sct_mu.pt(), sct_mu.eta(),
                                            sct_mu.phi(), sct_mu.m());
    ...
    for (const auto& sct_mu_second : *sctMuons) {
      ...
      math::PtEtaPhiMLorentzVector probe_sct_mu(
          sct_mu_second.pt(), sct_mu_second.eta(), sct_mu_second.phi(),
          sct_mu_second.m());
      if (sct_mu_second.pt() < 1) continue;
      ...
      if ((2.4 < invMass && invMass < 3.8)) {
        ...
        if (runWithoutVtx_) {
          ...
          if (scoutingMuonID(sct_mu_second)) {
            fillHistograms_resonance(histos.resonanceJ_numerator, sct_mu_second,
                                     vertex, invMass, -99.);
          }
          // Add all events to the denominator
          fillHistograms_resonance(histos.resonanceJ_denominator, sct_mu_second,
                                   vertex, invMass, -99.);
        } else {
          if (vtxIndx_tag.empty() || vtxIndx_probe.empty()) continue;
          for (const auto& commonIdx : vtxIndx_tag) {
            if (std::find(vtxIndx_probe.begin(), vtxIndx_probe.end(),
                          commonIdx) != vtxIndx_probe.end()) {
              const auto& vertex = (*sctVertex)[commonIdx];
              float lxy =
                  sqrt(vertex.x() * vertex.x() + vertex.y() * vertex.y());
              if (scoutingMuonID(sct_mu_second)) {
                fillHistograms_resonance(histos.resonanceJ_numerator,
                                         sct_mu_second, vertex, invMass, lxy);
              }
              fillHistograms_resonance(histos.resonanceJ_denominator,
                                       sct_mu_second, vertex, invMass, lxy);
            }
          }
        }
...
// Tag ID
bool ScoutingMuonTagProbeAnalyzer::scoutingMuonID(
    const Run3ScoutingMuon mu) const {
  ...
  if (mu.pt() > pt_threshold && fabs(mu.eta()) < eta_threshold &&
      mu.normalizedChi2() < normchisq_threshold && mu.isGlobalMuon() &&
      mu.nTrackerLayersWithMeasurement() > layer_threshold) {
    return true;
  }
  return false;
}
...
// Fill histograms
void ScoutingMuonTagProbeAnalyzer::fillHistograms_resonance(
    const kProbeKinematicMuonHistos histos, const Run3ScoutingMuon mu,
    const Run3ScoutingVertex vertex, const float inv_mass,
  ...
  histos.hnStandAloneMuonMatchedStations->Fill(
      mu.nStandAloneMuonMatchedStations());
  ...
  histos.hnRecoMuonExpectedMatchedStations->Fill(
      mu.nRecoMuonExpectedMatchedStations());
...
// Save histograms
void ScoutingMuonTagProbeAnalyzer::bookHistograms(
    DQMStore::IBooker& ibook, edm::Run const& run,
    edm::EventSetup const& iSetup, kTagProbeMuonHistos& histos) const {
  ibook.setCurrentFolder(outputInternalPath_);
  bookHistograms_resonance(ibook, run, iSetup, histos.resonanceJ_numerator,
                           "resonanceJ_numerator");
  bookHistograms_resonance(ibook, run, iSetup, histos.resonanceJ_denominator,
                           "resonanceJ_denominator");
}

// Set axes labels and range
void ScoutingMuonTagProbeAnalyzer::bookHistograms_resonance(
    DQMStore::IBooker& ibook, edm::Run const& run,
    edm::EventSetup const& iSetup, kProbeKinematicMuonHistos& histos,
    const std::string& name) const {
  ibook.setCurrentFolder(outputInternalPath_);

  histos.hPt = ibook.book1D(name + "_Probe_sctMuon_Pt",
                            name + "_Probe_sctMuon_Pt;Muon pt (GeV); Muons", 60,
                            0, 50.0);
  histos.hEta =
      ibook.book1D(name + "_Probe_sctMuon_Eta",
                   name + "_Probe_sctMuon_Eta; Muon eta; Muons", 60, -5.0, 5.0);
  histos.hPhi =
      ibook.book1D(name + "_Probe_sctMuon_Phi",
                   name + "_Probe_sctMuon_Phi; Muon phi; Muons", 60, -3.3, 3.3);
  histos.hInvMass = ibook.book1D(
      name + "_sctMuon_Invariant_Mass",
      name + "_sctMuon_Invariant_Mass;Muon Inv mass (GeV); Muons", 100, 0, 5);
  histos.hNormChisq = ibook.book1D(
      name + "_Probe_sctMuon_NormChisq",
      name + "_Probe_sctMuon_NormChisq; Muon normChi2; Muons", 60, 0, 5.0);
  histos.hTrk_dxy = ibook.book1D(
      name + "_Probe_sctMuon_Trk_dxy",
      name + "_Probe_sctMuon_Trk_dxy; Track dxy; Muons", 60, 0, 5.0);
  histos.hTrk_dz = ibook.book1D(name + "_Probe_sctMuon_Trk_dz",
                                name + "_Probe_sctMuon_Trk_dz; Track dz; Muons",
                                60, 0, 20.0);
  histos.htype =
      ibook.book1D(name + "_Probe_sctMuon_type",
                   name + "_Probe_sctMuon_type;Muon type; Muons", 15, 0, 15);
  histos.hcharge = ibook.book1D(
      name + "_Probe_sctMuon_charge",
      name + "_Probe_sctMuon_charge; Muon charge; Muons", 3, -1, 2);
  histos.hecalIso = ibook.book1D(
      name + "_Probe_sctMuon_ecalIso",
      name + "_Probe_sctMuon_ecalIso; Muon ecalIso; Muons", 60, 0.0, 20.0);
  histos.hhcalIso = ibook.book1D(
      name + "_Probe_sctMuon_hcalIso",
      name + "_Probe_sctMuon_hcalIso; Muon hcalIso; Muons", 60, 0.0, 20.0);
  histos.htrackIso =
      ibook.book1D(name + "_sctMuon_trackIso",
                   name + "_sctMuon_trackIso;Muon trackIso; Muons", 100, 0, 7);
  histos.hnValidStandAloneMuonHits =
      ibook.book1D(name + "_Probe_sctMuon_nValidStandAloneMuonHits",
                   ...
                   25, 0, 25);
  histos.hnValidRecoMuonHits = ibook.book1D(
      name + "_Probe_sctMuon_nValidRecoMuonHits",
      name + "_Probe_sctMuon_nValidRecoMuonHits;nValidRecoMuonHits; Muons", 25,
      0, 25);
  histos.hnRecoMuonChambers = ibook.book1D(
      name + "_Probe_sctMuon_nRecoMuonChambers",
      name + "_Probe_sctMuon_nRecoMuonChambers; nRecoMuonChambers; Muons", 10,
      0, 10.0);
  ...
  histos.hnRecoMuonChambersCSCorDT =
      ibook.book1D(name + "_Probe_sctMuon_nRecoMuonChambersCSCorDT",
                   ...
                   10, 0, 10.0);
  histos.hnRecoMuonMatches = ibook.book1D(
      name + "_Probe_sctMuon_nRecoMuonMatches",
      name + "_Probe_sctMuon_nRecoMuonMatches; nRecoMuonMatches; Muons", 6, 0,
      6);
  ...
  histos.hnRecoMuonExpectedMatchedStations =
      ibook.book1D(name + "_sctMuon_nRecoMuonExpectedMatchedStations",
                   ...
                   6, 0, 6.0);
  histos.hnValidPixelHits = ibook.book1D(
      name + "_Probe_sctMuon_nValidPixelHits",
      name + "_Probe_sctMuon_nValidPixelHits;nValidPixelHits; Muons", 14, 0,
      14.0);
  histos.hnValidStripHits = ibook.book1D(
      name + "_Probe_sctMuon_nValidStripHits",
      name + "_Probe_sctMuon_nValidStripHits; nValidStripHits; Muons", 25, 0,
      25);
  histos.htrk_chi2 = ibook.book1D(
      name + "_Probe_sctMuon_trk_chi2",
      name + "_Probe_sctMuon_trk_chi2; trk_chi2; Muons", 60, 0.0, 20);
  histos.htrk_ndof =
      ibook.book1D(name + "_sctMuon_trk_ndof",
                   name + "_sctMuon_trk_ndof;trk_ndof; Muons", 100, 0, 50);
  histos.htrk_lambda = ibook.book1D(
      name + "_Probe_sctMuon_trk_lambda",
      name + "_Probe_sctMuon_trk_lambda; trk_lambda; Muons", 60, -5, 5);
  histos.htrk_pt =
      ibook.book1D(name + "_Probe_sctMuon_trk_pt",
                   name + "_Probe_sctMuon_trk_pt; trk_pt; Muons", 60, 0, 50.0);
  histos.htrk_eta = ibook.book1D(name + "_Probe_sctMuon_trk_eta",
                                 name + "_Probe_sctMuon_trk_eta;trk_eta; Muons",
                                 60, -3.3, 3.3);
  histos.htrk_dxyError = ibook.book1D(
      name + "_Probe_sctMuon_trk_dxyError",
      name + "_Probe_sctMuon_trk_dxyError; trk_dxyError; Muons", 60, 0.0, 0.8);
  histos.htrk_dzError = ibook.book1D(
      name + "_Probe_sctMuon_trk_dzError",
      name + "_Probe_sctMuon_trk_dzError; trk_dzError; Muons", 60, 0.0, 2);
  histos.htrk_qoverpError = ibook.book1D(
      name + "_sctMuon_trk_qoverpError",
      name + "_sctMuon_trk_qoverpError;trk_qoverpError; Muons", 100, 0, 0.05);
  histos.htrk_lambdaError = ibook.book1D(
      name + "_Probe_sctMuon_trk_lambdaError",
      name + "_Probe_sctMuon_trk_lambdaError; trk_lambdaError; Muons", 60, 0,
      0.2);
  histos.htrk_phiError = ibook.book1D(
      name + "_Probe_sctMuon_trk_phiError",
      name + "_Probe_sctMuon_trk_phiError; trk_phiError; Muons", 60, 0, 0.2);
  histos.htrk_dsz = ibook.book1D(
      name + "_Probe_sctMuon_trk_dsz",
      name + "_Probe_sctMuon_trk_dsz; trk_dsz; Muons", 60, -20.0, 20.0);
  histos.htrk_dszError = ibook.book1D(
      name + "_Probe_sctMuon_trk_dszError",
      name + "_Probe_sctMuon_trk_dszError; trk_dszError; Muons", 60, 0, 0.8);
  histos.htrk_vx =
      ibook.book1D(name + "_Probe_sctMuon_trk_vx",
                   name + "_Probe_sctMuon_trk_vx; trk_vx; Muons", 60, -1, 1);
  histos.htrk_vy =
      ibook.book1D(name + "_Probe_sctMuon_trk_vy",
                   name + "_Probe_sctMuon_trk_vy; trk_vy; Muons", 60, -1, 1);
  histos.htrk_vz =
      ibook.book1D(name + "_Probe_sctMuon_trk_vz",
                   name + "_Probe_sctMuon_trk_vz; trk_vz; Muons", 60, -20, 20);
  histos.hnPixel = ibook.book1D(
      name + "_Probe_sctMuon_nPixel",
      name + "_Probe_sctMuon_nPixel; n Pixel Layers; Muons", 6, 0, 6);
  histos.hnTracker = ibook.book1D(
      name + "_Probe_sctMuon_nTracker",
      name + "_Probe_sctMuon_nTracker; n Tracker Layers; Muons", 14, 0, 25);
  histos.htrk_qoverp = ibook.book1D(
      name + "_Probe_sctMuon_trk_qoverp",
      name + "_Probe_sctMuon_trk_qoverp; n Track q/p; Muons", 40, -1, 1);
  histos.hLxy = ibook.book1D(name + "_Vertex_Lxy",
                             name + "_Vertex_Lxy; lxy; Muons", 60, 0, 20);
  histos.hXError =
      ibook.book1D(name + "_Vertex_Xerror",
                   name + "_Vertex_Xerror; vertex x error; Muons", 60, 0, 1);
  histos.hYError =
      ibook.book1D(name + "_Vertex_Yerror",
                   name + "_Vertex_Yerror; vertex y error; Muons", 60, 0, 1);
  histos.hChi2 =
      ibook.book1D(name + "_Vertex_chi2",
                   name + "_Vertex_chi2; vertex chi2; Muons", 60, 0, 15);
  histos.hZ = ibook.book1D(name + "_Vertex_z",
                           name + "_Vertex_z; vertex z; Muons", 60, 0, 15);
  histos.hx = ibook.book1D(name + "_Vertex_x",
                           name + "_Vertex_x; vertex x; Muons", 60, -1, 1);
  histos.hy = ibook.book1D(name + "_Vertex_y",
                           name + "_Vertex_y; vertex y; Muons", 60, -1, 1);
  histos.hZerror =
      ibook.book1D(name + "_Vertex_z error",
                   name + "_Vertex_z_error; vertex z error; Muons", 60, 0, 3);
  histos.htracksSize = ibook.book1D(
      name + "_Vertex_tracksSize",
      name + "_Vertex_tracksSize; vertex tracksSize; Muons", 60, 0, 10);
}
