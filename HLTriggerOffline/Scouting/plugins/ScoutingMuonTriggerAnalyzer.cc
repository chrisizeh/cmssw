...
ScoutingMuonTriggerAnalyzer::ScoutingMuonTriggerAnalyzer(
    ...
    : outputInternalPath_(
      ...
      triggerCache_(triggerExpression::Data(
          ...
          consumesCollector())),
      vtriggerSelection_(
          iConfig.getParameter<vector<string>>("triggerSelection")) {
  ...
  algToken_ = consumes<BXVector<GlobalAlgBlk>>(
      iConfig.getParameter<edm::InputTag>("AlgInputTag"));
  l1GtUtils_ = std::make_shared<l1t::L1TGlobalUtil>(
      iConfig, consumesCollector(), l1t::UseEventSetupIn::RunAndEvent);
...
void ScoutingMuonTriggerAnalyzer::analyze(edm::Event const& iEvent,
  ...
  if (triggerCache_.setEvent(iEvent, iSetup)) {
    for (unsigned int i = 0; i < vtriggerSelector_.size(); i++) {
      ...
      if (result) passHLTDenominator = true;
    }
  }
  ...
  if (!sctMuons->empty()) {
    ...
    std::sort(std::begin(sorted_mu), std::end(sorted_mu),
              [&](Run3ScoutingMuon mu1, Run3ScoutingMuon mu2) {
                return mu1.pt() > mu2.pt();
              });
    leading_mu = sorted_mu.at(0);
    if (sorted_mu.size() > 1) subleading_mu = sorted_mu.at(1);

    l1GtUtils_->retrieveL1(iEvent, iSetup, algToken_);

    math::PtEtaPhiMLorentzVector mu1(leading_mu.pt(), leading_mu.eta(),
                                     leading_mu.phi(), leading_mu.m());
    math::PtEtaPhiMLorentzVector mu2(subleading_mu.pt(), subleading_mu.eta(),
                                     subleading_mu.phi(), subleading_mu.m());
...
void ScoutingMuonTriggerAnalyzer::bookHistograms(
    DQMStore::IBooker& ibook, edm::Run const& run,
    edm::EventSetup const& iSetup) {
  ibook.setCurrentFolder(outputInternalPath_);
  h_invMass_denominator = ibook.book1D(
      "h_invMass_denominator", ";Invariant Mass (GeV); Muons", 100, 0.0, 20.0);
  h_pt1_l1_denominator = ibook.book1D(
      "h_pt1_denominator", ";Leading muon pt (GeV); Muons", 100, 0, 50.0);
  h_eta1_l1_denominator = ibook.book1D(
      "h_eta1_denominator", ";Leading muon eta; Muons", 100, -5.0, 5.0);
  h_phi1_l1_denominator = ibook.book1D(
      "h_phi1_denominator", ";Leading muon phi; Muons", 100, -3.3, 3.3);
  h_dxy1_l1_denominator = ibook.book1D("h_dxy1_denominator",
                                       ";Leading muon dxy; Muons", 100, 0, 5.0);
  h_pt2_l1_denominator = ibook.book1D(
      "h_pt2_denominator", ";Subleading muon pt (GeV); Muons", 100, 0, 50.0);
  h_eta2_l1_denominator = ibook.book1D(
      "h_eta2_denominator", ";Subleading muon eta; Muons", 100, -5.0, 5.0);
  h_phi2_l1_denominator = ibook.book1D(
      "h_phi2_denominator", ";Subleading muon phi; Muons", 100, -3.3, 3.3);
  h_dxy2_l1_denominator = ibook.book1D(
      "h_dxy2_denominator", ";Subleaing muon dxy; Muons", 100, 0, 5.0);

  for (unsigned int i = 0; i < l1Seeds_.size(); i++) {
    const auto& l1seed = l1Seeds_.at(i);
    h_invMass_numerators.push_back(
        ibook.book1D(Form("h_invMass_numerator_%s", l1seed.c_str()),
                     ";Invariant mass (GeV); Muons", 100, 0.0, 20.0));
    h_pt1_l1_numerators.push_back(
        ibook.book1D(Form("h_pt1_numerator_%s", l1seed.c_str()),
                     ";Leading muon pt (GeV); Muons", 100, 0, 50.0));
    h_eta1_l1_numerators.push_back(
        ibook.book1D(Form("h_eta1_numerator_%s", l1seed.c_str()),
                     ";Leading muon eta; Muons", 100, -5.0, 5.0));
    h_phi1_l1_numerators.push_back(
        ibook.book1D(Form("h_phi1_numerator_%s", l1seed.c_str()),
                     ";Leading muon phi; Muons", 100, 3.3, -3.3));
    h_dxy1_l1_numerators.push_back(
        ibook.book1D(Form("h_dxy1_numerator_%s", l1seed.c_str()),
                     ";Leading muon dxy; Muons", 100, 0, 5.0));
    h_pt2_l1_numerators.push_back(
        ibook.book1D(Form("h_pt2_numerator_%s", l1seed.c_str()),
                     ";Subleading muon pt (GeV); Muons", 100, 0, 50.0));
    h_eta2_l1_numerators.push_back(
        ibook.book1D(Form("h_eta2_numerator_%s", l1seed.c_str()),
                     ";Subleading muon eta; Muons", 100, -5.0, 5.0));
    h_phi2_l1_numerators.push_back(
        ibook.book1D(Form("h_phi2_numerator_%s", l1seed.c_str()),
                     ";Subleading muon phi; Muons", 100, 3.3, -3.3));
    h_dxy2_l1_numerators.push_back(
        ibook.book1D(Form("h_dxy2_numerator_%s", l1seed.c_str()),
                     ";Subleading muon dxy; Muons", 100, 0, 5.0));
  }
}
