...
TEST_CASE("SoAGenericView") {
  ...
  SECTION("Generic View") {
    ...
    genericView.xPos()[3] = 0.;
    REQUIRE(genericView.xPos()[3] == positionConstView.x()[3]);
  }
  ...
  SECTION("Generic ConstView from Views") {
    ...
    positionView.x()[3] = 0.;
    REQUIRE(genericConstView.xPos()[3] == positionView.x()[3]);
  }

  SECTION("Deep copy the Generic View") {
    // building the Layout
    const std::size_t customBufferSize = GenericSoA::computeDataSize(elems);
    std::unique_ptr<std::byte, decltype(std::free) *> bufferCustom{
        reinterpret_cast<std::byte *>(aligned_alloc(GenericSoA::alignment, customBufferSize)), std::free};
    GenericSoA genericSoA(bufferCustom.get(), elems);
    ...
    genericSoAView.xPos()[3] = 0.;
    REQUIRE(genericSoAView.xPos()[3] != positionView.x()[3]);
  }
}
